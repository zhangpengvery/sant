...
Page({
  ...
  changeTab:function(e){
    ...
    this.setData({
      ...
      if(that.data.currentIndex==0){
        that.getNoSignLists2(1,that.u_type)
      }else if(that.data.currentIndex==1){
        that.getSignLists2(1,that.u_type)
      }
    })
  },
  ...
  async getNoSignLists(page,u_type) {
    var that=this
  ...
  async getNoSignLists2(page,u_type) {
    var that=this
    let result = await requestApi(app.globalData.post_url + "/index.php/Api/LetSign/getNoSignLists",{
      page:page,
      u_type:u_type
  ...
  async getSignLists(page,u_type) {
    var that=this
  ...
  async getSignLists2(page,u_type) {
    var that=this
    let result = await requestApi(app.globalData.post_url + "/index.php/Api/LetSign/getSignLists",{
      page:page,
      u_type:u_type
  ...
  loadMore(){
    this.setData({
      page:++this.data.page
		...
		this.getNoSignLists(this.data.page,this.u_type)
  },
  loadMore2(){
    this.setData({
      page:++this.data.page
    })
		this.getSignLists(this.data.page,this.u_type)
  },
  /**
   * 生命周期函数--监听页面加载
   */
